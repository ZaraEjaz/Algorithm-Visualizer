...
// for the grid "isBlocked" will show whether there is a wall or not on the given cell.
...
/// create grid-like structure using a table.
const init = () => {
  const allowDrop = (e) => {
    e.preventDefault();
  };
...
document.getElementById('generateMazeButton').addEventListener('click', generateMaze);
...
function generateMaze() {
  ...
  // You can use a random algorithm to generate walls or customize as per your needs
  // Example: Randomly block some cells to create a maze-like structure
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      if (Math.random() < 0.3) { // Adjust the probability as needed
        gridElements[i][j].className = 'clicked';
        isBlocked[i][j] = 1;
      }
    }
  }
